...
static void _set_console_info(void)
{
    ...
    hProcess = OpenProcess(PROCESS_ALL_ACCESS, PDC_FALSE, dwConsoleOwnerPid);
    ...
    DuplicateHandle(GetCurrentProcess(), hSection, hProcess, &hDupSection,
                    0, PDC_FALSE, DUPLICATE_SAME_ACCESS);
...
static PDC_BOOL _fit_console_window(HANDLE con_out, CONST SMALL_RECT *rect)
{
    SMALL_RECT run;
    SHORT mx, my;

    if (SetConsoleWindowInfo(con_out, PDC_TRUE, rect))
        return PDC_TRUE;
    ...
    if (!SetConsoleWindowInfo(con_out, PDC_TRUE, &run))
        return PDC_FALSE;
...
void PDC_reset_prog_mode(void)
{
    ...
    if (is_nt)
    {
        ...
        SetConsoleWindowInfo(pdc_con_out, PDC_TRUE, &rect);
        SetConsoleScreenBufferSize(pdc_con_out, bufsize);
        SetConsoleActiveScreenBuffer(pdc_con_out);
    }
...
void PDC_reset_shell_mode(void)
{
    ...
    if (is_nt)
    {
        ...
        SetConsoleWindowInfo(pdc_con_out, PDC_TRUE, &orig_scr.srWindow);
        SetConsoleScreenBufferSize(pdc_con_out, orig_scr.dwSize);
        SetConsoleWindowInfo(pdc_con_out, PDC_TRUE, &orig_scr.srWindow);
        SetConsoleActiveScreenBuffer(pdc_con_out);
    }
