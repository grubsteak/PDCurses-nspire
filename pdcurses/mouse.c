...
/*man-start**************************************************************
  ...
  Synopsis:
        ...
        PDC_bool wenclose(const WINDOW *win, int y, int x);
        PDC_bool wmouse_trafo(const WINDOW *win, int *y, int *x, PDC_bool to_screen);
        PDC_bool mouse_trafo(int *y, int *x, PDC_bool to_screen);
  ...
  Description:
        ...
        relative coordinates. A to_screen parameter of PDC_TRUE means to
        convert from window to screen; otherwise the reverse. The
        function returns PDC_FALSE if the coordinates aren't within the
        window, or if any of the parameters are NULL. The coordinates
        have been converted when the function returns PDC_TRUE.
...
static PDC_bool ungot = PDC_FALSE;
