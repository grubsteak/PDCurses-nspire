...
/*man-start**************************************************************
  ...
  Description:
        ...
        isendwin() returns PDC_TRUE if endwin() has been called without a 
        ...
        is_termresized() returns PDC_TRUE if the curses screen has been
...
WINDOW *Xinitscr(int argc, char *argv[])
{
    ...
    SP->autocr = PDC_TRUE;       /* cr -> lf by default */
    SP->raw_out = PDC_FALSE;     /* tty I/O modes */
    SP->raw_inp = PDC_FALSE;     /* tty I/O modes */
    SP->cbreak = PDC_TRUE;
    SP->save_key_modifiers = PDC_FALSE;
    SP->return_key_modifiers = PDC_FALSE;
    SP->echo = PDC_TRUE;
    SP->visibility = 1;
    ...
    if (SP->_preserve)
    {
        untouchwin(curscr);
        untouchwin(stdscr);
        stdscr->_clear = PDC_FALSE;
        curscr->_clear = PDC_FALSE;
    }
    else
        curscr->_clear = PDC_TRUE;
