...
/*man-start**************************************************************
  ...
  Synopsis:
        ...
        PDC_bool is_linetouched(WINDOW *win, int line);
        PDC_bool is_wintouched(WINDOW *win);
  ...
  Description:
        ...
        is_linetouched() returns PDC_TRUE if the specified line in the 
        specified window has been changed since the last call to 
        wrefresh().

        is_wintouched() returns PDC_TRUE if the specified window 
...
PDC_bool is_linetouched(WINDOW *win, int line)
{
    ...
    return (win->_firstch[line] != _NO_CHANGE) ? PDC_TRUE : PDC_FALSE;
}
