...
/*man-start**************************************************************
  ...
  Synopsis:
        ...
        int syncok(WINDOW *win, PDC_bool bf);
  ...
  Description:
        ...
        If wsyncok() is called with a second argument of PDC_TRUE, this
...
WINDOW *PDC_makenew(int nlines, int ncols, int begy, int begx)
{
    ...
    win->_clear = (PDC_bool) ((nlines == LINES) && (ncols == COLS));
    win->_bmarg = nlines - 1;
...
WINDOW *resize_window(WINDOW *win, int nlines, int ncols)
{
    ...
    if (!(win->_flags & (_SUBPAD|_SUBWIN)))
    {
        ...
        copywin(win, new, 0, 0, 0, 0, min(win->_maxy, new->_maxy) - 1,
                min(win->_maxx, new->_maxx) - 1, PDC_FALSE);
